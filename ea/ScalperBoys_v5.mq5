//+------------------------------------------------------------------+
//|                                           ScalperBoys_v5.mq5      |
//|   EA v5 STANDALONE (tanpa server). HYBRID + budget per-leg.       |
//|                                                                  |
//|   >>> VERSI 5.0 (config aktif) = HYBRID (InpEntryMode=2) <<<      |
//|     * CHOPPY (gap EMA <=1.5xATR): mean-rev fade RSI BUY<=35 SELL>=65|
//|     * TREN KUAT (gap >1.5xATR)  : trend-pullback, entry pas RSI    |
//|       silang InpPullLevel(52) searah tren (beli dip, jual bounce)  |
//|     * BUDGET TERPISAH: 2 mean-rev + 2 trend (InpMaxPosPerLeg)      |
//|     * Filter: news ON + sesi 05-21 GMT ; SL ATR x1.2 anti-spike    |
//|     * TP: SL x RR (1:2) ; TANPA batas harian                      |
//|                                                                  |
//|   Backtest: WR 46%, +$91/bln, DD ~$17, Prof/DD 5.2 (paling efisien|
//|   dari semua opsi). Budget per-leg bikin posisi nyangkut 1 leg    |
//|   tak ngejegal leg lain, TANPA nambah drawdown.                   |
//|                                                                  |
//|   Changelog:                                                     |
//|     2.3 = mean-rev murni RSI35/65 (Prof/DD 5.6).                   |
//|     4.x = HYBRID: +leg trend-pullback pas tren kuat.               |
//|     5.0 = budget TERPISAH per-leg (2 MR + 2 TR). +$91/bln DD$17    |
//|           Prof/DD 5.2. InpEntryMode=1 utk balik mean-rev murni.    |
//|                                                                  |
...
input int    InpEntryMode    = 2;       // 0=trend-pullback, 1=mean-rev, 2=HYBRID v4 (dipakai)
...
input double InpPullLevel    = 52.0;    // (hybrid v4) leg-tren kuat: entry pas RSI silang level ini searah tren (RR pakai InpRR)
...
int OnInit()
{
   ...
   Print("ScalperBoys_v5 aktif di ", _Symbol, " — lot ", InpLot, " RR 1:", InpRR);
...
void OnTick()
{
   ...
   else if(InpEntryMode==2)  // HYBRID v4: choppy=mean-rev, tren-kuat=trend-pullback(cross)
   {
      ...
      else                 // TREN KUAT -> trend-pullback: RSI silang PULL searah tren
      {
         int trend = (emaF>emaS) ? 1 : ((emaF<emaS) ? -1 : 0);
         if(trend == 0) return;
         bool crossUp   = (rsiPrev <  InpPullLevel        && rsi >= InpPullLevel);
         bool crossDown = (rsiPrev > (100.0-InpPullLevel) && rsi <= (100.0-InpPullLevel));
         wantBuy  = (trend==1  && crossUp);
         wantSell = (trend==-1 && crossDown);
      }
   }
