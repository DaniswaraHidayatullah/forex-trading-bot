//+------------------------------------------------------------------+
//|                                           ScalperBoys_v6.mq5      |
//|   EA v6 STANDALONE (tanpa server). MEAN-REV + BREAKOUT.           |
//|                                                                  |
//|   >>> VERSI 6.0 (config aktif) = MR+BREAKOUT (InpEntryMode=2) <<< |
//|     * CHOPPY (gap EMA <=1.5xATR): mean-rev fade RSI BUY<=35 SELL>=65|
//|       SL anti-spike, RR 1:2 (InpRR).                              |
//|     * TREN KUAT (gap >1.5xATR)  : BREAKOUT tembus high/low 20-bar  |
//|       searah tren (impuls, BUKAN beli dip). SL ATR polos, RR 1:3.  |
//|     * Filter: news ON + sesi 05-21 GMT (overlap 13-16 = tambang!)  |
//|     * Max 2 posisi ; TANPA batas harian                           |
//|                                                                  |
//|   Backtest ~1.7bln: WR 43%, +$100/bln, DD ~$22, profit TIAP bulan.|
//|   Breakout (impuls) survive tren gold zigzag di mana trend-follow  |
//|   (beli dip) gagal. 40+ strategi diuji: MR+Breakout satu2nya lolos.|
//|                                                                  |
//|   Changelog:                                                     |
//|     2.3 = mean-rev murni RSI35/65 (Prof/DD 5.6, paling aman).      |
//|     4.x/5.x = HYBRID trend-pullback -> RUGI di tren zigzag gold.   |
//|     6.0 = MR + BREAKOUT (impuls). InpEntryMode=1 utk MR murni aman.|
//|                                                                  |
...
input int    InpEntryMode    = 2;       // 0=trend-pullback, 1=mean-rev, 2=MR+BREAKOUT v6 (dipakai)
...
input double InpPullLevel    = 52.0;    // (lama v4/v5, tak dipakai v6)
input int    InpBreakoutN    = 20;      // (v6) leg-tren kuat: BREAKOUT tembus high/low brp bar M15
input double InpBreakoutRR   = 3.0;     // (v6) RR leg breakout (breakout butuh TP gede: RR 1:3)
...
input int    InpMaxPositions = 4;       // maks posisi TOTAL (backstop = 2 MR + 2 BO)
input int    InpMaxPosPerLeg = 2;       // maks posisi PER-STRATEGI (2 MR + 2 Breakout, jatah sendiri)
...
int OnInit()
{
   ...
   Print("ScalperBoys_v6 aktif di ", _Symbol, " — lot ", InpLot, " RR 1:", InpRR);
...
//    "MR" = mean-rev, "BO" = breakout. Budget terpisah tiap leg.
...
void OnTick()
{
   ...
   else if(InpEntryMode==2)  // MR+BREAKOUT v6: choppy=mean-rev, tren-kuat=breakout
   {
      ...
      legTag = strong ? "BO" : "MR";        // choppy=mean-rev leg, tren kuat=BREAKOUT
      if(!strong)          // CHOPPY -> mean-rev (fade 35/65)
      {
         wantBuy  = (rsi <= InpRsiLo);
         wantSell = (rsi >= InpRsiHi);
      }
      else                 // TREN KUAT -> BREAKOUT: tembus high/low N-bar searah tren
      {
         int trend = (emaF>emaS) ? 1 : ((emaF<emaS) ? -1 : 0);
         if(trend == 0) return;
         double bh[], bl[];                                  // high/low 20 bar SEBELUM bar tutup
         if(CopyHigh(_Symbol,PERIOD_M15,2,InpBreakoutN,bh) < InpBreakoutN) return;
         if(CopyLow (_Symbol,PERIOD_M15,2,InpBreakoutN,bl) < InpBreakoutN) return;
         double close1 = iClose(_Symbol,PERIOD_M15,1);       // close bar tutup
         double rngHi  = bh[ArrayMaximum(bh)];
         double rngLo  = bl[ArrayMinimum(bl)];
         wantBuy  = (trend==1  && close1 > rngHi);           // tembus HIGH N-bar (impuls)
         wantSell = (trend==-1 && close1 < rngLo);           // tembus LOW N-bar
      }
   }
   ...
   // SL: MR pakai ANTI-SPIKE (di luar wick 12 bar); BREAKOUT pakai ATR polos.
   ...
   double useRR  = (legTag == "BO") ? InpBreakoutRR : InpRR;   // BO=RR3, MR=RR2
   double tpDist = slDist * useRR;
   ...
   if(wantBuy)
   {
      ...
      ok = trade.Buy(lot, _Symbol, ask, sl, tp, "SBv6-"+legTag);
   }
   else
   {
      sl = NormalizeDouble(price + slDist, digits);
      tp = NormalizeDouble(price - tpDist, digits);
      ok = trade.Sell(lot, _Symbol, bid, sl, tp, "SBv6-"+legTag);
   }
